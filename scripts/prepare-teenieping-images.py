...
for profile in profiles:
    ...
    sections.append(f'<section><h2><img src="../../src/assets/teenie/{profile["id"]}.webp" alt="">{escape(profile["name"])}</h2><div class="clues">{"".join(cards)}</div></section>')
...
(ROOT / "docs/assets/teenieping-artwork.html").write_text(html.replace("READY_COUNT", str(len(ready))).replace("SECTIONS", "".join(sections)), encoding="utf-8")
